...
- `emplace_back` 和 `push_back` 的区别在于，`emplace_back` 能够接受放入对象的构造函数的参数，然后直接使用这些参数构造新的对象，免去了拷贝构造、移动构造等消耗。虽然 `push_back` 也能接受单个参数构造函数的参数，但是其机制在于类型自动转换隐式地调用了对象的单参数构造函数，然后生产临时对象，然后再进行移动构造，临时对象的析构等额外步骤，增加了开销。
